...
import { billSchema, type BillFormValues } from "../../(common)/schemas";
import { Button, Input } from "@/(common)/components/ui";
import { Card, CardContent } from "@/(common)/components/ui";
import { useAction } from "next-safe-action/hooks";
import { saveBillDraft, approveBill, generateNextBillNumber } from "../actions";
import { useRouter } from "next/navigation";
import { useState, useEffect } from "react";
import { calculateBillTotals } from "../../(common)/utils";
...
export function BillForm({ vendors, expenseAccounts, apAccounts, initialData }: Props) {
  ...
  useEffect(() => {
    if (!initialData) {
      generateNextBillNumber({})
        .then(({ data }) => {
          if (data?.billNumber) {
            form.setValue("bill_number", data.billNumber);
          }
        })
        .catch(console.error);
    }
  ...
  const totals = calculateBillTotals(lines);

  // Actions
  const saveDraft = useAction(saveBillDraft, {
    onSuccess: ({ data }) => {
      if (data?.billId) {
        setCurrentId(data.billId);
        setIsDraftSaved(true);
      }
    }
  });

  const approve = useAction(approveBill, {
    onSuccess: () => {
  ...
  const handleApprove = () => {
    if (!apAccounts.length) {
      alert("No Accounts Payable account found. Please create one in Chart of Accounts.");
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="space-y-2">
                ...
                {form.formState.errors.vendor_id && (
                  <p className="text-red-500 text-sm">{form.formState.errors.vendor_id.message}</p>
                )}
              </div>

              <div className="space-y-2">
                <label className="block text-sm font-medium">Bill #</label>
                <Input {...form.register("bill_number")} />
                {form.formState.errors.bill_number && (
                  <p className="text-red-500 text-sm">{form.formState.errors.bill_number.message}</p>
                )}
              </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <h3 className="text-lg font-medium mb-4">Bill Line Items</h3>
            <div className="space-y-3">
              {fields.map((field, index) => (
                <div key={field.id} className="flex gap-2 items-end">
                  ...
                  <div className="w-48">
                    <label className="text-xs text-gray-500">Expense/Asset Account</label>
                    <select
          ...
          <div className="flex gap-4">
            ...
            {isDraftSaved && (
              <Button
                ...
                disabled={approve.status === 'executing'}
              >
                {approve.status === 'executing' ? 'Approving...' : 'Approve Bill'}
              </Button>
            )}
          </div>
        </div>
