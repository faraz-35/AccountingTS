...
export default async function BalanceSheetPage({
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ...
      ...rows.map(row => [row.account_code, row.account_name, row.account_type, row.amount.toString()])
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-end print:hidden">
        ...
        <form className="flex gap-2 items-end">
          <div>
            <Label htmlFor="date" className="text-xs font-bold">As Of Date</Label>
            <Input type="date" id="date" name="date" defaultValue={asOfDate} required />
          </div>
      ...
      <div className="bg-white shadow rounded-lg p-6 print:shadow-none">
        <h2 className="text-xl font-bold mb-4 pb-2 border-b">Balance Sheet Summary</h2>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center mb-6">
          <div className="p-4 bg-blue-50 rounded-lg">
            <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">Total Assets</h3>
            <p className="text-2xl font-bold text-blue-600">
              {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(totalAssets)}
            </p>
          </div>
          <div className="p-4 bg-orange-50 rounded-lg">
            <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">Total Liabilities</h3>
            <p className="text-2xl font-bold text-orange-600">
              {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(totalLiabilities)}
            </p>
          </div>
          <div className="p-4 bg-green-50 rounded-lg">
            <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">Total Equity</h3>
        ...
        <div className="border-t-2 border-gray-200 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-center">
            <div className="p-4">
              <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">Assets</h3>
              <p className="text-xl font-bold">
                {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(totalAssets)}
              </p>
            </div>
            <div className="p-4">
              <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">Liabilities + Equity</h3>
              <p className="text-xl font-bold">
                {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(totalLiabilitiesAndEquity)}
              </p>
            </div>
          </div>
        </div>
        ...
        {!isBalanced && (
          <div className="mt-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
            ...
            <span className="block sm:inline">
              Balance Sheet is out of balance by {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(balanceDifference)}.
