...
export default async function TrialBalancePage({
  ...
  try {
    ...
    const { data: summary, error: summaryError } = await supabase.rpc("get_trial_balance_summary", {
      p_organization_id: null, // Will be filtered by RLS
      p_as_of_date: asOfDate,
    });
    ...
    const groupedAccounts = trialBalance?.reduce(
      (acc, account) => {
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Verify that debits equal credits as of {new Date(asOfDate).toLocaleDateString()}
            </p>
          </div>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    Total Debits: {formatCurrency(summaryData?.total_debits || 0)} |
                    Total Credits: {formatCurrency(summaryData?.total_credits || 0)}
                  </p>
                </div>
              </div>
              <div className="text-right">
                ...
                >
                  {isBalanced ? "✓ Valid" : `⚠ Diff: ${formatCurrency(Math.abs(summaryData?.difference || 0))}`}
                </Badge>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-center text-red-600">
              ...
              <h3 className="text-lg font-semibold">Error Loading Trial Balance</h3>
              <p className="text-gray-600">
